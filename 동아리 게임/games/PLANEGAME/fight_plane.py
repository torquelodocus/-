...
enemy_kill_count = 0  # 적 처치 횟수 카운터
                ...
                for n in range(MISSILE_MAX):
                    if msl_f[n] == True and get_dis(emy_x[i], emy_y[i], msl_x[n], msl_y[n]) < r * r:
                        ...
                        if emy_shield[i] <= 0:
                            ...
                            if emy_type[i] != EMY_BOSS:
                                enemy_kill_count += 1
                                if enemy_kill_count >= 3 and ss_shield < 100:
                                    ss_shield += 1
                                    enemy_kill_count = 0
                            emy_f[i] = False
    ...
    while True:
        ...
        if idx == 2:  # 게임 오버
            ...
            if tmr >= 117:
                ...
                btn = tk.Button(root, text="기록 전송", font=("malgungothic", int(screen_height*0.035), "bold"), bg="#323232", fg="#fff", activebackground="#00bfff", activeforeground="#fff", relief="flat", command=on_submit)
