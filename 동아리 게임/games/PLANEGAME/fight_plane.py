        ...
        for i in range(ENEMY_MAX):
            if emy_f[i] == True:
                ...
                if get_dis(emy_x[i], emy_y[i], ss_x, ss_y) < r * r:
                    # 충돌 시 폭발 효과를 설정합니다. (set_effect 함수는 따로 정의되어 있어야 함)
                    ...
                    if ss_muteki == 0:
                        ss_muteki = 40 # 무적 상태 시간 설정 (60프레임)
...
def move_missile(scrn): # 탄환 이동
    ...
    movement_angle = 0
    if ss_d == 0:
        movement_angle = 270 - 0 # 0도 방향
    elif ss_d == 1:
        movement_angle = 270 - 0 # 30도 방향
    elif ss_d == 2:
        movement_angle = 270 - 0 # 330도 방향
    else:
        pass
    ss_d -= 180
...
def main():  # 메인 루프
    ...
    se_barrage = pygame.mixer.Sound(rsrc("sound_plane/barrage.ogg"))
    ...
    while True:
        ...
        if idx == 2:  # 게임 오버
            ...
            # 2.5초(75프레임) 후에 Tkinter 기록 입력 창 실행
            if tmr >= 117:
                pygame.quit()
                ...
                def on_submit():
                    ...
                    if not name:
                        ...
                        return
                    send_to_server(name)
                    messagebox.showinfo("전송 완료", "기록이 서버로 전송되었습니다.")
                    root.destroy()
                    import sys
                    ...
                    sys.exit()
                root = tk.Tk()
                root.title("GAME LAUNCHER - 기록 전송")
                ...
                btn = tk.Button(root, text="기록 전송 및 종료", font=("malgungothic", int(screen_height*0.035), "bold"), bg="#323232", fg="#fff", activebackground="#00bfff", activeforeground="#fff", relief="flat", command=on_submit)
                btn.pack(ipady=int(screen_height*0.015), ipadx=int(screen_width*0.03))
                btn.bind("<Enter>", on_enter)
                btn.bind("<Leave>", on_leave)
                entry_name.bind('<Return>', lambda event: on_submit())  # 엔터키로도 전송
                root.mainloop()
        ...
        if idx == 3:  # 게임 클리어
            ...
            if tmr >= 117:
                ...
                screen_height = root.winfo_screenheight()
                # 타이틀
                title = tk.Label(root, text="입력", font=("malgungothic", int(screen_height*0.05), "bold"), fg="#00bfff", bg="#1e1e1e")
                title.pack(pady=(int(screen_height*0.08), int(screen_height*0.03)))
                # 안내
                info = tk.Label(root, text="학번 이름을 입력하세요:", font=("malgungothic", int(screen_height*0.03)), fg="#fff", bg="#1e1e1e")
                info.pack(pady=(0, int(screen_height*0.02)))
        ...
        # draw_text(screen, "HISCORE " + str(hisco), 760, 30, 50, CYAN)  # 이 줄을 삭제 또는 주석 처리
        if idx != 0:  # 실드 표시
            screen.blit(img_shield, [40, 680])
# 빨간 실드 막대: x 위치는 40, 너비는 ss_shield * 4
